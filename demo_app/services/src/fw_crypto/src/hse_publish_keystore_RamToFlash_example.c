...
{
    publishKeystoreRamtoFlashStatus publishKeystoreRamtoFlashStatus_t = PUBLISH_KEYSTORE_RAM_TO_FLASH_NOT_STARTED;
    hseSrvResponse_t srvResponse = HSE_SRV_RSP_GENERAL_ERROR;
    ...
    */
    ...
    ASSERT(HSE_SRV_RSP_OK == srvResponse);
    stmTime_before_publish_key_srvid = 0;
    EnableStm();
    srvResponse = ImportNVMkeysAPI(); //importing the NVM keys
    ASSERT(HSE_SRV_RSP_OK == srvResponse);
    DisbleStm();
    stmTime_before_publish_key_srvid = MeasureStm();
    ...
    */
    ...
    stmTime_after_publish_key_srvid = 0;
    EnableStm();
    ...
    publishKeystoreRamtoFlashStatus_t = PUBLISH_KEYSTORE_RAM_TO_FLASH_STARTED;
    srvResponse = HSE_Publish_Keystore_RamtoFlash(); // This will now update the keys to data flash also
    ASSERT(HSE_SRV_RSP_OK == srvResponse);

    DisbleStm();
    stmTime_after_publish_key_srvid = MeasureStm();

    if(stmTime_after_publish_key_srvid < stmTime_before_publish_key_srvid )
    {
        publishKeystoreRamtoFlashStatus_t = PUBLISH_KEYSTORE_RAM_TO_FLASH_SUCCESSFUL;
    }
    else
    {
        publishKeystoreRamtoFlashStatus_t = PUBLISH_KEYSTORE_RAM_TO_FLASH_UNSUCCESSFUL;
    }

    if(publishKeystoreRamtoFlashStatus_t == PUBLISH_KEYSTORE_RAM_TO_FLASH_SUCCESSFUL)
    {
    ...
    {
        return publishKeystoreRamtoFlashStatus_t;
    }
